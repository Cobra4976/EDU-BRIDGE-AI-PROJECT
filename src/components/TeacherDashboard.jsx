...
//         {["ai-assistant", "student-progress", "assignments", "community"].includes(activeTab) && (
//           <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg">
//             <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
//               {{
//                 "ai-assistant": "🤖 AI Teaching Assistant",
//                 "student-progress": "📈 Student Progress Monitor",
//                 "assignments": "📝 Assignment Manager",
//                 "community": "👥 Teacher Community Finder",
//               }[activeTab]}
//             </h2>
...
//             <textarea
//               placeholder={`Ask Edubridge AI about ${activeTab.replace("-", " ")}... Example: "Create a lesson plan for teaching fractions" or "How do I support struggling learners?"`}
//               value={aiPrompt}
//               onChange={(e) => setAiPrompt(e.target.value)}
//               className="w-full border border-gray-300 rounded-lg px-4 py-3 mb-4 focus:ring-2 focus:ring-[#2e7d32] outline-none min-h-[120px] text-gray-700"
//             />

//             <button onClick={() => generateAIResponse(aiPrompt, activeTab)} disabled={aiLoading || !isOnline}
//               className="bg-[#2e7d32] hover:bg-[#43a047] text-white px-6 py-3 rounded-lg font-medium transition disabled:bg-gray-400 disabled:cursor-not-allowed">
//               {!isOnline ? "🔌 Offline - AI Unavailable" : aiLoading ? "🤔 EduBridge is thinking..." : "✨ Ask Edubridge AI"}
//             </button>
...
//  *
//  * Behavior:
//  *  - If teacherProfile prop supplied, uses it as initial state
//  *  - Otherwise fetches /teachers/{uid} from Firestore
//  *  - Save profile writes to /teachers/{uid} (setDoc merge)
//  *  - Keeps all original UI + features (AI prompt, tabs)
//  */
// export default function TeacherDashboard({ user, teacherProfile: initialProfile = null, onLogout }) {
//   // user is required (should be provided by App.jsx)
//   // Local UI/profile state
//   const [teacherProfile, setTeacherProfile] = useState(
//     initialProfile || {
//       name: "",
//       country: "",
...
//   const [loadingProfile, setLoadingProfile] = useState(!initialProfile); // if no initialProfile, fetch
//   const [profileError, setProfileError] = useState("");

//   const [activeTab, setActiveTab] = useState("overview");

//   // Form editing state (editable copy)
//   const [profileData, setProfileData] = useState({
//     name: initialProfile?.name || "",
//     email: user?.email || "",
//     country: initialProfile?.country || "",
...
//   // Map & curriculum constants (unchanged)
//   const educationalSystems = {
//     "Kenya": ["CBC (Competency-Based Curriculum)", "8-4-4 System (Legacy)"],
//     "Nigeria": ["6-3-3-4 System", "Universal Basic Education (UBE)"],
//     "South Africa": ["CAPS (Curriculum and Assessment Policy)", "National Senior Certificate (NSC)"],
...
//   // If initialProfile changes (prop update), sync it
//   useEffect(() => {
//     if (initialProfile) {
//       setTeacherProfile(initialProfile);
//       setProfileData((prev) => ({
...
//   // Fetch profile from /teachers/{uid} if no initialProfile provided
//   useEffect(() => {
//     let cancelled = false;
//     async function fetchProfile() {
//       if (!user?.uid) return;
//       if (initialProfile) return; // already provided
//       setLoadingProfile(true);
//       setProfileError("");
//       try {
//         const teacherRef = doc(db, "teachers", user.uid);
...
//       // Use setDoc with merge to allow create or update
//       await setDoc(teacherRef, updatedProfile, { merge: true });
...
//   // AI assistant generation (keeps your existing approach)
//   const generateAIResponse = async (prompt, contextType) => {
//     if (!prompt || !prompt.trim()) {
//       alert("Please enter a question or request.");
//       return;
...
//     try {
//       // NOTE: This expects a backend proxy at /api/claude (CORS). Keep your existing backend proxy.
//       const response = await fetch('http://localhost:3001/api/claude', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ systemPrompt, userPrompt, maxTokens: 4096 })
...
//   // Real logout uses passed callback if available
//   const handleLogout = async () => {
//     if (onLogout) {
//       setIsLoggingOut(true);
//       try {
...
//       }
//     } else {
//       // If no onLogout provided, do nothing but log
//       console.warn("onLogout callback not provided. Provide a function that calls signOut(auth).");
//     }
//   };

//   // Theming (green + white) — kept from your earlier theme decisions
//   const containerBg = "bg-white";
//   const primary = "text-[#2e7d32]";
//   const primaryBg = "bg-[#2e7d32] hover:bg-[#43a047] text-white";
//   const muted = "text-gray-700";

//   // Loading / profile errors handling UI
//   if (loadingProfile) {
//     return (
//       <div className="min-h-screen flex items-center justify-center bg-white">
//         <div className="text-center">
...
//   if (profileError) {
//     // Friendly message when teacher profile not created
//     return (
//       <div className="min-h-screen flex items-center justify-center bg-white px-6">
//         <div className="w-full max-w-lg bg-white border border-green-100 p-8 rounded-xl shadow">
//           <h2 className="text-2xl font-semibold mb-4 text-[#2e7d32]">Profile missing</h2>
...
//             <button onClick={() => window.location.reload()} className="px-4 py-2 rounded bg-[#2e7d32] text-white">Reload</button>
//             <button onClick={() => {
//               // provide a helpful alternative: navigate back or sign out
//               if (onLogout) onLogout();
//             }} className="px-4 py-2 rounded border border-green-300 text-[#2e7d32]">Sign Out</button>
//           </div>
//         </div>
//       </div>
//     );
...
//   // Main dashboard UI (profile loaded)
//   return (
//     <div className="min-h-screen bg-white">
//       <nav className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center shadow-sm">
//         <div className="flex items-center gap-3">
...
//               <p className="text-sm">
//                 {isOnline 
//                   ? 'All features are available. Your profile and data are syncing.' 
//                   : 'Limited features available. Some data may not sync until you reconnect.'}
//               </p>
//             </div>
...
//                   <li className="flex items-start gap-2">
//                     <span className="text-orange-600">⚠</span>
//                     <span>AI features require internet connection</span>
//                   </li>
//                   <li className="flex items-start gap-2">
//                     <span className="text-orange-600">⚠</span>
//                     <span>Profile updates will sync when connection is restored</span>
...
//  *  - Otherwise fetches /teachers/{uid} from Firestore
//  *  - Save profile writes to /teachers/{uid} (setDoc merge)
//  *  - Keeps all original UI + features (AI prompt, upload simulation, tabs)
//  */
// export default function TeacherDashboard({ user, teacherProfile: initialProfile = null, onLogout }) {
//   // user is required (should be provided by App.jsx)
//   // Local UI/profile state
...
//     "assignments": "Help create curriculum-aligned assignments, rubrics, and assessment strategies",
//     "community": "Connect teachers with professional networks and development opportunities in their country",
//     "students": "Provide classroom management strategies for large African classes and student engagement tips"
//   };
...
//       return;
//     }
//     // Simulation only: you might instead upload to storage and write metadata to teacherContent collection
//     alert(`✅ ${file.name} uploaded successfully (simulation).`);
//     setFile(null);
//   };
...
//               { id: "community", label: "Teacher Community", icon: "👥" },
//               { id: "students", label: "Total Students", icon: "🎓" },
//             ].map((tab) => (
//               <button
//                 key={tab.id}
//                 onClick={() => {
...
//         {["ai-assistant", "student-progress", "assignments", "community", "students"].includes(activeTab) && (
//           <div className="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-lg">
//             <h2 className="text-2xl font-bold text-gray-800 mb-4 flex items-center">
//               {{
//                 "ai-assistant": "🤖 AI Teaching Assistant",
...
//             <button onClick={() => generateAIResponse(aiPrompt, activeTab)} disabled={aiLoading}
//               className="bg-[#2e7d32] hover:bg-[#43a047] text-white px-6 py-3 rounded-lg font-medium transition disabled:bg-gray-400 disabled:cursor-not-allowed">
//               {aiLoading ? "🤔 EduBridge is thinking..." : "✨ Ask Edubridge AI"}
//             </button>
...
//             <h2 className="text-3xl font-bold text-gray-800 mb-6">📤 Upload Teaching Materials</h2>
//             <p className="text-gray-600 mb-6">Upload lesson plans, worksheets, or educational resources to share with your professional network.</p>
//             <input type="file" onChange={handleFileUpload}
//               className="border border-gray-300 p-3 rounded-lg w-full mb-4" />
//             {file && <p className="text-sm text-gray-600 mb-4">Selected: <strong>{file.name}</strong></p>}
...
//         <p className="font-semibold">🌍 EduBridge Africa - Powered by Edubridge AI</p>
//         <p className="mt-1">Supporting SDG 4: Quality Education • Curriculum-aware AI for African educators</p>
//         <p className="mt-2 text-xs">Using AI for contextual educational assistance</p>
//       </footer>
//     </div>
//   );
// }

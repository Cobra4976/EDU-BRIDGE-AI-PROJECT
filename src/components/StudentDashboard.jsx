...
//       // Craft a careful JSON-output prompt so we can parse results
//       const prompt = `
// You are an educational assistant. Generate 4 practical study tasks for a single student based on the profile below.
...
//       // Try to extract JSON (some models return text with explanation)
//       const jsonTextMatch = text.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
//       const jsonText = jsonTextMatch ? jsonTextMatch[0] : text;
...
//       // Save to Firestore (students doc)
//       const ok = await persistStudentData({ tasks: generatedTasks });
//       if (!ok) throw new Error('Failed to persist tasks to Firestore');

//       setTasks(generatedTasks);
//       // ADD THIS: Cache tasks offline
//     await saveTasksWithAnswers(user.uid, generatedTasks);
//     console.log('✅ Tasks cached offline');
...
//     const prompt = `
// You are an educational assessment assistant for African students. Based on the student's profile below,
// estimate TWO categories of skills:
...
//       // Provide tasks and skills context if available
//       const tasksContext = JSON.stringify(tasks || []);
//       const skillsContext = JSON.stringify(skills || {});

//       const prompt = `
// You are an achievement engine. Create up to 5 achievement objects for this student based on the profile, tasks, and skills.
...
//       setAchievements(generatedAchievements);
//       // ADD THIS: Cache achievements offline
//       await saveAchievements(user.uid, generatedAchievements);
//     console.log('✅ Achievements cached offline');
...
//     const prompt = `
// You are an educational content creator specializing in ${isTechSkill ? 'technology education for African students' : 'academic skill development'}. 

// Generate a personalized learning path for this student to improve their ${skillName} skill from ${currentScore}% proficiency to mastery.
...
//       } 
//     });
//     // / ADD THIS: Cache learning path offline
//     // ADD THIS: Cache learning path offline
//     await saveLearningPath(user.uid, skillName, {
//       category,
//       startedAt: new Date().toISOString(),
...
//       const context = `You are a personalized AI tutor for ${studentProfile?.name || 'a student'} who is studying ${studentProfile?.educationalSystem || 'their curriculum'} in ${studentProfile?.country || 'their country'}.
...
//     // ADD THIS: Cache chat messages offline
//     await saveChatMessages(user.uid, updatedMessages);
//     console.log('✅ Chat messages cached offline');
//     }catch (error) {
//       console.error('AI Error:', error);
//       let msg = 'I apologize, but I encountered an error.';
//       if (!import.meta.env.VITE_GOOGLE_API_KEY) {
//         msg += ' API key is missing. Please add VITE_GOOGLE_API_KEY to your .env file.';
//       } else {
//         msg += ` ${error.message || 'Unknown error.'}`;
//       }
//       setChatMessages(prev => [...prev, { role: 'assistant', content: msg }]);
...
//     const recentActivities = activityLog.slice(-10); // Last 10 activities
    
//     const prompt = `
// You are an AI educational advisor for African students. Based on the student's profile and progress, recommend 3-5 skills they should focus on next.
...
//               {chatMessages.slice(-10).reverse().map((msg, idx) => (
//                 <div key={idx} className={`p-3 rounded ${
//                   msg.role === 'user' ? 'bg-green-100 ml-8' : 'bg-blue-100 mr-8'
//                 }`}>
//                   <div className="text-xs font-semibold text-gray-600 mb-1">
...
//                   Showing last 10 of {chatMessages.length} messages
//                 </p>
//               )}
//             </div>
//           ) : (
...
//             {skills.academic && Object.keys(skills.academic).length > 0 && (
//               <div>
//                 <div className="text-sm font-semibold text-blue-600 mb-2">📚 Academic Skills</div>
//                 <div className="grid grid-cols-2 gap-2">
//                   {Object.entries(skills.academic).map(([skill, score]) => (
//                     <div key={skill} className="bg-white p-2 rounded border border-blue-200">
//                       <div className="text-xs font-medium text-gray-800">{skill}</div>
//                       <div className="text-xs text-blue-600 font-bold">{score}%</div>
//                     </div>
//                   ))}
//                 </div>
//               </div>
...
//             {skills.technology && Object.keys(skills.technology).length > 0 && (
//               <div>
//                 <div className="text-sm font-semibold text-green-600 mb-2">💻 Technology Skills</div>
//                 <div className="grid grid-cols-2 gap-2">
//                   {Object.entries(skills.technology).map(([skill, score]) => (
//                     <div key={skill} className="bg-white p-2 rounded border border-green-200">
//                       <div className="text-xs font-medium text-gray-800">{skill}</div>
//                       <div className="text-xs text-green-600 font-bold">{score}%</div>
//                     </div>
//                   ))}
//                 </div>
//               </div>
...
//             Profile Information
//           </summary>
//           <div className="mt-4 bg-white p-4 rounded border border-gray-200 space-y-2">
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Name:</span>
//               <span className="text-sm text-gray-800 ml-2">{studentProfile?.name || 'N/A'}</span>
//             </div>
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Email:</span>
//               <span className="text-sm text-gray-800 ml-2">{user?.email || 'N/A'}</span>
//             </div>
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Country:</span>
//               <span className="text-sm text-gray-800 ml-2">{studentProfile?.country || 'N/A'}</span>
//             </div>
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Educational System:</span>
//               <span className="text-sm text-gray-800 ml-2">{studentProfile?.educationalSystem || 'N/A'}</span>
//             </div>
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Strengths:</span>
//               <div className="text-sm text-gray-800 mt-1 bg-green-50 p-2 rounded border border-green-200">
//                 {studentProfile?.strengths || 'Not specified'}
//               </div>
//             </div>
//             <div>
//               <span className="text-xs font-semibold text-gray-600">Areas for Improvement:</span>
//               <div className="text-sm text-gray-800 mt-1 bg-blue-50 p-2 rounded border border-blue-200">
//                 {studentProfile?.weaknesses || 'Not specified'}
//               </div>
//             </div>
//           </div>
...
//         {/* Tasks Tab - enhanced with Gemini */}
//         {activeTab === 'tasks' && (
//           <div className="max-w-3xl mx-auto">
//             <div className="bg-white rounded-xl shadow-lg p-8">
//               <div className="flex items-center justify-between mb-6">
...
//               {aiErrorTasks && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4">{aiErrorTasks}</div>}

//               {tasks.length === 0 ? (
//                 <div className="text-center p-12 text-gray-600">
//                   <p>No tasks yet. Click "Generate Tasks (AI)" to create tailored tasks based on your profile.</p>
//                 </div>
//               ) : (
//                 <div className="space-y-4">
//                   {tasks.map((t, idx) => (
...
//                       <div className="flex justify-between items-start">
//                         <h3 className="text-lg font-semibold">{t.title || `Task ${idx+1}`}</h3>
//                         <div className="text-sm text-gray-500">{t.difficulty || 'Medium'}</div>
//                       </div>
//                       <p className="mt-2 text-gray-700">{t.description}</p>
//                       {t.estimatedMinutes && <div className="mt-2 text-sm text-gray-500">Est: {t.estimatedMinutes} minutes</div>}
//                     </div>
//                   ))}
//                 </div>
//               )}
...
//                     className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg"
//                   >
//                     {aiLoadingAchievements ? 'Generating...' : 'Generate Achievements (AI)'}
//                   </button>
//                 </div>
//               </div>

//               {aiErrorAchievements && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4">{aiErrorAchievements}</div>}

//               {achievements.length === 0 ? (
//                 <div className="text-center p-12 text-gray-600">
//                   <p>No achievements yet. Click "Generate Achievements (AI)" to create milestone templates.</p>
//                 </div>
//               ) : (
...
//                         </div>
//                         <div className="text-sm text-gray-500">{a.date ? new Date(a.date).toLocaleDateString() : 'Not awarded'}</div>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//         <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
//           <div className="text-sm font-semibold text-blue-600 mb-1">Tasks Cached</div>
//           <div className="text-2xl font-bold text-blue-800">{tasks.length}</div>
//           <div className="text-xs text-blue-600 mt-1">Available offline</div>
//         </div>

//         <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
//           <div className="text-sm font-semibold text-purple-600 mb-1">Skills Tracked</div>
//           <div className="text-2xl font-bold text-purple-800">{Object.keys(skills).length}</div>
//           <div className="text-xs text-purple-600 mt-1">Available offline</div>
//         </div>

//         <div className="bg-green-50 p-4 rounded-lg border border-green-200">
//           <div className="text-sm font-semibold text-green-600 mb-1">Achievements</div>
//           <div className="text-2xl font-bold text-green-800">{achievements.length}</div>
//           <div className="text-xs text-green-600 mt-1">Available offline</div>
//         </div>
//       </div>
...
//               <p className="text-sm text-yellow-800">
//                 ⚠️ You have {syncStatus.pendingCount} change{syncStatus.pendingCount > 1 ? 's' : ''} waiting to sync. 
//                 They will be uploaded automatically when you're back online.
//               </p>
//             </div>
//           )}
//         </div>
...
//                 <div key={idx} className="bg-white p-3 rounded border border-gray-200">
//                   <div className="font-medium text-gray-800">{task.title}</div>
//                   <div className="text-sm text-gray-600 mt-1">{task.difficulty} • {task.estimatedMinutes} min</div>
//                 </div>
//               ))}
//               {tasks.length > 3 && (
...
//             </div>
//           ) : (
//             <p className="text-gray-600 text-sm">No tasks cached yet. Generate tasks while online to access them offline.</p>
//           )}
//         </div>
...
//               {Object.entries(skills).slice(0, 6).map(([skill, score]) => (
//                 <div key={skill} className="bg-white p-3 rounded border border-gray-200">
...
//                       ></div>
//                     </div>
//                     <span className="ml-2 text-xs text-gray-600">{score}%</span>
//                   </div>
//                 </div>
//               ))}
//             </div>
//           ) : (
//             <p className="text-gray-600 text-sm">No skills cached yet. Analyze skills while online to track them offline.</p>
//           )}
//         </div>
...
//                 <p className="text-sm text-gray-600 text-center mt-2">
//                   + {achievements.length - 3} more achievements available
//                 </p>
//               )}
//             </div>
//           ) : (
//             <p className="text-gray-600 text-sm">No achievements cached yet. Generate achievements while online to view them offline.</p>
//           )}
//         </div>
...
//             Your conversation history with the AI tutor is automatically saved and available offline.
//             {chatMessages.length > 1 && ` You have ${chatMessages.length} messages in your history.`}
//           </p>
//         </div>
//       </div>
...
//         </h4>
//         <ul className="text-sm text-blue-700 space-y-2">
//           <li>• All your data is automatically cached for offline access</li>
//           <li>• Changes made offline are queued and synced when you reconnect</li>
//           <li>• Tasks, skills, and achievements remain accessible without internet</li>
//           <li>• AI features require internet connection to generate new content</li>
//           <li>• Your progress is never lost, even if you go offline unexpectedly</li>
//         </ul>
//       </div>
//     </div>
//   </div>
...
//   // 2) Analyze skills and return skill scores (0-100)
//   const analyzeSkills = async () => {
//     setAiErrorSkills('');
//     setAiLoadingSkills(true);
...
//       const prompt = `
// You are an educational assessment assistant. Based on the student's profile below,
// estimate a set of 6 relevant skills and give each a score from 0 to 100 indicating current competency.
// Return the result strictly as JSON object with skill names as keys and integer scores as values.
...
//               {aiErrorSkills && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4">{aiErrorSkills}</div>}

//               {(!skills || Object.keys(skills).length === 0) ? (
//                 <div className="text-center p-12 text-gray-600">
//                   <p>No skill data yet. Click "Analyze Skills (AI)" to estimate skill levels from your profile.</p>
//                 </div>
//               ) : (
//                 <div className="space-y-4">
//                   {Object.entries(skills).map(([skill, score]) => (
//                     <div key={skill} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
//                       <div className="flex justify-between items-center">
//                         <div className="font-semibold text-gray-800">{skill}</div>
//                         <div className="text-sm text-gray-600">{score}%</div>
//                       </div>
//                       <div className="mt-2 h-3 bg-gray-200 rounded-full overflow-hidden">
//                         <div style={{ width: `${Math.max(0, Math.min(100, Number(score || 0)))}%` }} className="h-3 bg-green-500 rounded-full"></div>
//                       </div>
//                     </div>
//                   ))}
//                 </div>
...
//         {/* Offline Content (unchanged) */}
//         {activeTab === 'content' && (
//           <div className="max-w-3xl mx-auto">
//             <div className="bg-white rounded-xl shadow-lg p-12 text-center">
//               <div className="text-6xl mb-4">📚</div>
//               <h2 className="text-3xl font-bold text-gray-800 mb-3">Offline Content</h2>
//               <p className="text-gray-600 text-lg">This feature is coming soon. Download study materials and resources here.</p>
//             </div>
//           </div>
//         )}
//       </main>
